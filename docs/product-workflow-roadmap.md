...
- The mixed-provider selected-download ordering defect was fixed before
  release, preserving the JSON attribution contract intended for native
  consumers such as Incant Audio; that integration remains unverified.
...
- Keep the supported 0.4.0 claim to Linux CLI/API use on Python 3.10–3.13.
  Label Python 3.14, other operating systems, and desktop MCP hosts
  experimental until their exact combinations have recorded evidence.
...
The Claude Desktop `download_manifest`/shutdown trial and Incant Audio
end-to-end handoff remain P1 validation work. They must pass before those
integrations are advertised as supported, but do not block the narrower Linux
CLI/API beta.
