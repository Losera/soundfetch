"""soundfetch ML dataset exports — build HuggingFace datasets, WebDataset
shards, and attribution files from a soundfetch manifest.

All three functions consume ``manifest.jsonl`` (the dataset index written
by ``soundfetch <provider> search`` / ``soundfetch <provider> download``)
and only consider records with ``status == "downloaded"`` and a
``local_file`` that exists on disk.

Heavy libraries (``datasets``, ``webdataset``, ``soundfile``) are imported
**inside** each function so the base ``soundfetch`` package never pulls
them in::

    pip install "soundfetch[export]"  # pulls datasets + webdataset + soundfile
...
--------
    ...
    from soundfetch.export import to_hf_dataset
    ds = to_hf_dataset("out/manifest.jsonl")
    ds.push_to_hub("my-audio-dataset")
...
__all__ = ["to_hf_dataset", "to_webdataset", "export_attribution"]
...
def to_hf_dataset(
    ...
    """Build a HuggingFace ``datasets.Dataset`` from downloaded sounds.

    The resulting dataset exposes an ``Audio`` feature column plus scalar
    metadata columns.  When *out_dir* is given the dataset is also saved
    to disk at that location.

    Args:
        manifest: Path to the ``manifest.jsonl`` file.
        dest_dir: Base directory for resolving relative ``local_file``
            values.  Defaults to the manifest's parent directory.
        out_dir: Optional directory to persist the dataset via
    ...
    import datasets  # lazy — only pulled when this function is called
    ...
    for rec in records:
        data["audio"].append({
            "path": str(rec["local_path"]),
            "array": None,
        })
        data["sound_id"].append(rec.get("provider_id", ""))
        data["name"].append(rec.get("name", ""))
        data["provider"].append(rec.get("provider", ""))
        data["license"].append(rec["license_text"])
        data["author"].append(rec["author"])
        data["duration"].append(rec["duration"])
        data["samplerate"].append(rec["samplerate"])
        data["gen_ai"].append(rec["gen_ai"])
        data["url"].append(rec.get("url", ""))

    features = datasets.Features({
        "audio": datasets.Audio(),
        "sound_id": datasets.Value("string"),
        "name": datasets.Value("string"),
        "provider": datasets.Value("string"),
        "license": datasets.Value("string"),
        "author": datasets.Value("string"),
        "duration": datasets.Value("float32"),
        "samplerate": datasets.Value("float32"),
        "gen_ai": datasets.Value("string"),
