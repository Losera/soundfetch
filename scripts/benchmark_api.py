...
def _render_figures(frame, summaries, output_dir: Path, plt) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 4.8))
    ax.bar(mean_wall.index, mean_wall.values,
           color=[palette[provider] for provider in mean_wall.index], width=0.64)
    _configure_axes(
        ax,
            ...
            for trial in range(1, args.trials + 1):
                ...
                try:
                    source_rows, summary = _benchmark_source(
                        source, query=args.query, limit=args.limit,
                        output_dir=output_dir, run_label=label, workers=workers,
